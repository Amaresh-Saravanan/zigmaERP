...
| **Current Task** | TASK-B10 — Frontend `client.js` repointed per module (module-by-module cutover from PHP `crud.php` to Django). |
| **Last Completed Task** | TASK-B09: Reports module (`reports` app) — `Measurable`, `Logsheet`, `DC`, all on `MongoModelViewSet`/`HasScreenPermission`, same shape as `inventory`/`process` (B06/B08). **Two things corrected during a follow-up pass** (an earlier draft of this task used a hand-rolled viewset/serializer shape that didn't match this codebase's actual `MongoModelViewSet` contract and didn't run): (1) `DC` was initially modeled as a "Daily Concentration" report guess; checking the real frontend (`DCForm.jsx`) showed DC is actually a **Delivery Challan** — an invoice-style document with line items, GST, and tax rate — so the model was corrected to match, with server-computed `grand_total`. (2) The `*_Report` aggregate endpoints (MeasurableReport, EggProcessReport, PitStatusReport, RejectsReport) were dropped from scope after checking their frontend pages: `MeasurableReportList.jsx` etc. still call legacy PHP endpoints directly (e.g. `folders/pit_status/form.php`), so there's no Django contract to build against yet — that work belongs in TASK-B10 (frontend cutover) once each report page is actually repointed. `Logsheet` is a minimal placeholder model for the same reason: its frontend (`LogsheetList.jsx`) is a static print template with zero API calls. Routes: `/api/measurable`, `/api/logsheet`, `/api/dc`. Tests follow the established `mongomock` + `APIClient` pattern (not Django's `django_db` marker, since this project has no relational DB — `DATABASES = {}`). 6 new tests (69 total across the backend). |
...
| TASK-B10 | Frontend `client.js` repointed per module (module-by-module cutover) | Not Started | Each module's API task | Old `crud.php` removed only after this is verified. 9 Django modules now ready to repoint (Item/Unit/Tray/Pit/Supplier, User mgmt, all Process modules, Measurable/Logsheet/DC); `*_Report` pages stay on PHP until their aggregate endpoints exist. |
