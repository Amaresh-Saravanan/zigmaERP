...
| **Current Task** | TASK-B10 — Frontend `client.js` repointed per module (module-by-module cutover from PHP `crud.php` to Django). Core CRUD + User/UserType done; UserScreen/MainScreen (reduced mapping) and all Process modules remain (see §17.1 B10 row). |
| **Last Completed Task** | TASK-B10 (partial): User + UserType (List + Form each) repointed to Django, reusing the established `mode="django"` pattern, **verified live against Atlas**. Two field-boundary translations: frontend's `email` ↔ backend's `user_email`, and password is now genuinely optional on update (blank = keep existing hash) — the old PHP version's password field wasn't actually populated with anything real in edit mode, so this makes explicit what was previously silently broken. **`UserPermission` module skipped, not deferred** — it manages legacy's per-`UserType` screen assignment, and TASK-B07 deliberately moved permissions to per-`User` instead (`screens`/`main_screens` fields directly on `User`), so there is no Django endpoint this module could target; it isn't a gap in this cutover, the underlying feature doesn't exist in the new design. Left on PHP indefinitely unless the permission model changes. Live verification: create UserType → create User referencing it → retrieve → update without a password (confirmed hash preserved) → delete both. Full backend suite still 69/69. |
| **Overall Progress** | Frontend: 7/26 pages cut over to Django (Item, Unit, Tray, Pit, Supplier, User, UserType). Backend: TASK-B01–B09 done (9/14), TASK-B10 in progress. |
...
| TASK-B10 | Frontend `client.js` repointed per module (module-by-module cutover) | **In Progress** | Each module's API task | Built the shared plumbing (djangoClient.js, DataTable.jsx `mode="django"`, token auth alongside PHP session auth) and repointed **7 modules**: Core CRUD (Item, Unit, Tray, Pit, Supplier) + User, UserType. All verified end-to-end against live Atlas data via curl. Old `crud.php` **not yet removed** for any module — that only happens once each is also verified via real browser click-through, which needs a connected Chrome extension (not available in this environment). `UserPermission` **permanently skipped** — no Django endpoint exists for it (permissions moved to per-User in B07). Remaining: UserScreen/MainScreen (map to `core.Screen`/`core.MainScreen` but with fewer fields — legacy's `screen_type`/`order_no`/`description`/action-checkboxes don't all exist server-side), all Process modules, Measurable/Logsheet/DC. `*_Report` pages stay on PHP until their aggregate endpoints exist (deferred in B09). |
...
| User Management — UserPermission | PHP (legacy) | **Not planned to cut over** — no Django endpoint exists; permissions moved to per-User in B07, this legacy per-UserType module has no equivalent feature in the new design |
