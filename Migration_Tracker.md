...
| **Current Task** | TASK-B10 — Frontend `client.js` repointed per module (module-by-module cutover from PHP `crud.php` to Django). Core CRUD (Item/Unit/Tray/Pit/Supplier) all done; User management + Process modules remain (see §17.1 B10 row). |
| **Last Completed Task** | TASK-B10 (partial): all 5 Core CRUD modules — Item, Unit, Tray, Pit, Supplier (List + Form each) — repointed to Django and **verified end-to-end against live Atlas data**. Item/Unit established the pattern (see prior entry); Tray/Pit/Supplier reused it directly: `TRAY_TYPE_CHOICES` hardcoded client-side (matching the backend's own choices, since Django doesn't expose a choices endpoint) replacing the old PHP-form-scrape, Pit's client-side volume auto-calc kept alongside the server's authoritative computed value, Supplier's label/GST normalization logic carried over unchanged (server also normalizes, so it's belt-and-suspenders). **Live verification (2026-07-03)**: full curl CRUD cycles against the real cluster for all three — Tray (create/retrieve/update/delete, including a `tray_type` change), Pit (create with dimensions, confirmed server-computed `volume: 9.0` for 2×3×1.5), Supplier (create with lowercase `label`/`gst_no`, confirmed server-side uppercasing matches what the UI also does). Full backend suite still 69/69. **Still not verified**: pixel-level browser click-through (Chrome extension not connected in this environment) — the API contract itself is confirmed correct for all 5 modules via curl, so this is a lower-risk gap. |
| **Overall Progress** | Frontend: 5/26 pages cut over to Django (Item, Unit, Tray, Pit, Supplier — all of Core CRUD). Backend: TASK-B01–B09 done (9/14), TASK-B10 in progress. |
...
| TASK-B10 | Frontend `client.js` repointed per module (module-by-module cutover) | **In Progress** | Each module's API task | Built the shared plumbing (djangoClient.js, DataTable.jsx `mode="django"`, token auth alongside PHP session auth) and repointed all **5 Core CRUD modules — Item, Unit, Tray, Pit, Supplier** (List + Form each). All verified end-to-end against live Atlas data via curl (create/retrieve/update/delete per module, including Pit's server-computed `volume` and Supplier's server-side label/GST normalization). Old `crud.php` **not yet removed** for any module — that only happens once each is also verified via real browser click-through, which needs a connected Chrome extension (not available in this environment). Remaining: User mgmt, all Process modules, Measurable/Logsheet/DC — same pattern, now mostly repetition. `*_Report` pages stay on PHP until their aggregate endpoints exist (deferred in B09). |
