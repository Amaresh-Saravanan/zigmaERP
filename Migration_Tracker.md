...
| **Current Task** | TASK-B10 — Frontend `client.js` repointed per module (module-by-module cutover from PHP `crud.php` to Django). Item Creation + Unit Creation done as the first pair; 7 more Django-ready modules remain (see §17.1 B10 row). |
| **Last Completed Task** | TASK-B10 (partial): Item Creation + Unit Creation (List + Form) repointed to Django. This needed more than a URL swap — Django's `MongoModelViewSet` returns paginated JSON objects with token auth, while `DataTable.jsx` was built for PHP's array-of-pre-rendered-HTML-string rows with session-cookie auth. Added: `djangoClient.js` (new axios instance, JSON + `Authorization: Token` header, runs alongside the PHP `client.js` since both backends serve traffic mid-cutover), `interceptors.js` (toast/error handling factored out and shared by both clients), `DataTable.jsx` `mode="django"` (dot-path column keys like `unit.unit_name`, explicit `unique_id`, DRF's `{count, results}` shape), a best-effort Django token fetch added to `Login.jsx` alongside the existing PHP login (failure doesn't block login — Django's user store is separate and not every user exists there yet), and a `/api` proxy in `vite.config.js`. Backend: `core/mongo_viewset.py`'s pagination now accepts `?page_size=` so the "Show N entries" selector keeps working. Unit was done as the simple no-FK case, Item as the harder nested-reference case (unit dropdown + `unit.unit_name` display column) — both share the new `mode="django"` path, so the next 7 modules are now mostly plumbing, not design. Verified: full backend suite (69/69) green after the pagination change; Vite proxy confirmed forwarding `/api` to Django and returning 401 with no token. **Not verified**: live click-through in a browser (no Chrome extension connection this session) or an actual create/edit/delete against real data — blocked by the Atlas connectivity issue below. |
| **Overall Progress** | Frontend: Item/Unit Creation on Django (2/26 pages cut over), rest still PHP. Backend: TASK-B01–B09 done (9/14), TASK-B10 in progress. |
| **Active Blocker** | **MongoDB Atlas has a live SSL handshake failure from this machine** (`[SSL: TLSV1_ALERT_INTERNAL_ERROR]`), discovered 2026-07-03 while testing TASK-B10. This is new information superseding the earlier "connected" note below — that earlier check only called `mongoengine.connect()`, which is lazy and doesn't contact the server; it never actually proved connectivity. A real query (creating a test user, and pytest against live Atlas) now fails the TLS handshake consistently, including with an explicit `certifi` CA bundle (ruling out a stale-cert cause). Likely a network-level cause (firewall, VPN, or antivirus doing TLS inspection) on this machine, or an Atlas-side TLS config mismatch — not a credentials problem. Needs investigation outside this session; all current backend tests run against `mongomock` so this doesn't block development, only live-data verification and deployment. |
...
| TASK-B10 | Frontend `client.js` repointed per module (module-by-module cutover) | **In Progress** | Each module's API task | Built the shared plumbing (djangoClient.js, DataTable.jsx `mode="django"`, token auth alongside PHP session auth) and repointed **Item Creation + Unit Creation** as the first pair (List + Form each), proving both the simple case (Unit, no FK) and the harder case (Item, `unit` reference field). Old `crud.php` **not yet removed** — not verified end-to-end against live data yet (see Active Blocker: Atlas TLS issue). Remaining: Tray/Pit/Supplier, User mgmt, all Process modules, Measurable/Logsheet/DC — now mostly repetition of the same pattern. `*_Report` pages stay on PHP until their aggregate endpoints exist (deferred in B09). |
...
|--------|---------|-------|
| Item Creation | PHP (legacy) | Django endpoint (`mode="django"`) wired in parallel (TASK-B10), not yet verified against live data or cut over |
| Tray Creation | PHP (legacy) | |
| Pit Creation | PHP (legacy) | |
| Unit Creation | PHP (legacy) | Django endpoint (`mode="django"`) wired in parallel (TASK-B10), not yet verified against live data or cut over |
