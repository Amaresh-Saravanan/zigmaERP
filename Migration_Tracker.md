...
| **Current Phase** | Phase 1 (React): 100%. Phase 2 (UI Modernization): 100%. Phase 3 (Django Backend): TASK-B01–B05 done, TASK-B06 in progress (Item/Unit done; Tray/Pit/Supplier remain). Phase 4 (Deployment): 0%. |
| **Current Task** | TASK-B06 (remainder) — clone the Item/Unit pattern for Tray, Pit, Supplier. |
| **Last Completed Task** | TASK-B06: `core.mongo_viewset.MongoModelViewSet` — a hand-rolled CRUD base for MongoEngine Documents (DRF's ModelViewSet/GenericAPIView assume a Django ORM queryset; `get_object_or_404` reads `queryset.model.DoesNotExist`, which mongoengine querysets don't have, so the generic mixins would 500 instead of 404). `inventory.models.Unit`/`Item`, serializers, `ItemViewSet`/`UnitViewSet` wired through `HasScreenPermission` (TASK-B04's first real consumer) via `DefaultRouter` at `/api/units`, `/api/items`. Item auto-generates sequential codes (`IT-001`, ...). 9 new tests (27 total across the whole backend): permission-denied paths, create/list/retrieve/update/soft-delete lifecycle, duplicate-name rejection, unknown-reference rejection, search filtering. |
| **Overall Progress** | Frontend: 100% complete. Backend: TASK-B01–B05 done, B06 partial (5.5/14). |
...
| TASK-B06 | Core CRUD APIs (Item, Tray, Pit, Unit, Supplier) | **Partially Done** | TASK-B04 (done) | Item + Unit done as the reference implementation (`inventory` app), plus `core.mongo_viewset.MongoModelViewSet` — the reusable base every future module clones. **Tray, Pit, Supplier not yet built** — same pattern, straightforward clone. |
