...
| **Current Task** | TASK-B10 — Frontend `client.js` repointed per module (module-by-module cutover from PHP `crud.php` to Django). Core CRUD + User/UserType/UserPermission done; UserScreen/MainScreen (reduced mapping) and all Process modules remain (see §17.1 B10 row). |
| **Last Completed Task** | TASK-B10 (partial): `UserPermission` **restored and repointed** — reverses the earlier "skipped, no backend equivalent" call from the previous session. User feedback: the admin needs to assign access per role, not hand-edit each user's `screens` CSV individually; B07's per-User-only permission model missed that real requirement. Fix: `UserType` gained its own `screens`/`main_screens` fields (mirrors `User`'s existing fields) as a role's default permission set; `UserManageSerializer.create()` now has a new `User` inherit these from its `UserType` when not explicitly given (explicit values still override — tested both ways). `HasScreenPermission` itself is untouched — it still reads `User.screens` directly, so this only changes what a new user starts with, not how permission checks work. Also added `GET /api/permission-catalog`: every screen id any registered ViewSet actually enforces, grouped by router prefix and **derived from the router at request time** rather than hand-transcribed (the ~20 `required_screens` dicts across `accounts`/`core`/`inventory`/`process`/`reports` already define this uniformly as `<module>_view/create/edit/delete`; duplicating it into a static list would just drift out of sync as modules are added). Frontend `UserPermissionForm` now picks a `UserType` and renders a module × action checkbox grid from that catalog, plus a `MainScreen` checklist, saving back onto the `UserType` via PUT. **Live verification**: created a role, assigned `item_view/item_create/unit_view` via PUT, created a user of that role with no explicit screens, confirmed exact inheritance, then logged in as that user and confirmed items-list 200s while trays-list and item-delete both 403 — the inherited permissions gate real requests, not just stored data. Full backend suite 74/74 (+5 tests: permission round-trip, inheritance, override, catalog auth/content). |
| **Overall Progress** | Frontend: 8/26 pages cut over to Django (Item, Unit, Tray, Pit, Supplier, User, UserType, UserPermission). Backend: TASK-B01–B09 done (9/14), TASK-B10 in progress. |
...
| TASK-B10 | Frontend `client.js` repointed per module (module-by-module cutover) | **In Progress** | Each module's API task | Built the shared plumbing (djangoClient.js, DataTable.jsx `mode="django"`, token auth alongside PHP session auth) and repointed **8 modules**: Core CRUD (Item, Unit, Tray, Pit, Supplier) + User, UserType, UserPermission. All verified end-to-end against live Atlas data via curl. Old `crud.php` **not yet removed** for any module — that only happens once each is also verified via real browser click-through, which needs a connected Chrome extension (not available in this environment). `UserPermission` was initially skipped (see B10 UserType/User entry) on the reasoning that B07 moved permissions to per-User with no backend equivalent for per-role assignment — **user feedback reversed this**: admins need role-based assignment, so `UserType` gained its own `screens`/`main_screens` fields as a role's default set, new `User`s inherit from their `UserType` unless overridden, and a new `GET /api/permission-catalog` (derived from the router, not hand-maintained) drives the module × action checkbox grid. Remaining: UserScreen/MainScreen (map to `core.Screen`/`core.MainScreen` but with fewer fields — legacy's `screen_type`/`order_no`/`description`/action-checkboxes don't all exist server-side), all Process modules, Measurable/Logsheet/DC. `*_Report` pages stay on PHP until their aggregate endpoints exist (deferred in B09). |
...
| User Management — UserScreen | PHP (legacy) | Maps to `core.Screen` but with a reduced field set (no `description`, no action-checkboxes) — not yet repointed |
