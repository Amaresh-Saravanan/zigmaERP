...
| **Current Task** | TASK-B10 — Frontend `client.js` repointed per module (module-by-module cutover from PHP `crud.php` to Django). **All of User Management done** (User, UserType, UserPermission, UserScreen, MainScreen); all Process modules + Measurable/Logsheet/DC remain (see §17.1 B10 row). |
| **Last Completed Task** | TASK-B10 (partial): `MainScreen` + `UserScreen` repointed to Django, closing out User Management (5/5 sub-modules now cut over in parallel). Both map to real backend models (`core.MainScreen`, `core.Screen`) but with fewer fields than legacy's single flat admin page had: `MainScreen` drops `screen_type`/`order_no`/`description` (legacy's `screen_type` distinguished top vs sub-level within one table; Django already split that into two separate collections, so the field has no home on either side). `UserScreen` (→ `core.Screen`) drops the per-action checkboxes and `description` — action-level permissions now live on `UserType.screens` via the `UserPermission` module built in the prior step, not per-Screen. Live verification: create MainScreen → create Screen referencing it → retrieve (confirmed nested `main_screen`) → update → delete both. Full backend suite still 74/74 (no backend changes this round — pure frontend). |
| **Overall Progress** | Frontend: 10/26 pages cut over to Django (Item, Unit, Tray, Pit, Supplier, User, UserType, UserPermission, UserScreen, MainScreen — all of Core CRUD + User Management). Backend: TASK-B01–B09 done (9/14), TASK-B10 in progress. |
...
| TASK-B10 | Frontend `client.js` repointed per module (module-by-module cutover) | **In Progress** | Each module's API task | Built the shared plumbing (djangoClient.js, DataTable.jsx `mode="django"`, token auth alongside PHP session auth) and repointed **10 modules**: Core CRUD (Item, Unit, Tray, Pit, Supplier) + all of User Management (User, UserType, UserPermission, UserScreen, MainScreen). All verified end-to-end against live Atlas data via curl. Old `crud.php` **not yet removed** for any module — that only happens once each is also verified via real browser click-through, which needs a connected Chrome extension (not available in this environment). `UserPermission` was initially skipped, then restored per user feedback: `UserType` gained `screens`/`main_screens` as a role's default set, new `User`s inherit from their `UserType` unless overridden, and a new `GET /api/permission-catalog` (derived from the router, not hand-maintained) drives the module × action checkbox grid. `UserScreen`/`MainScreen` map to `core.Screen`/`core.MainScreen` with fewer fields than legacy (no `description`, no `order_no` on MainScreen, no per-Screen action-checkboxes — that's now UserPermission's job). Remaining: all Process modules, Measurable/Logsheet/DC. `*_Report` pages stay on PHP until their aggregate endpoints exist (deferred in B09). |
