...
| **Last Completed Task** | TASK-B10 (partial): Item Creation + Unit Creation (List + Form) repointed to Django, **now verified end-to-end against live Atlas data** (see below). This needed more than a URL swap — Django's `MongoModelViewSet` returns paginated JSON objects with token auth, while `DataTable.jsx` was built for PHP's array-of-pre-rendered-HTML-string rows with session-cookie auth. Added: `djangoClient.js` (new axios instance, JSON + `Authorization: Token` header, runs alongside the PHP `client.js` since both backends serve traffic mid-cutover), `interceptors.js` (toast/error handling factored out and shared by both clients), `DataTable.jsx` `mode="django"` (dot-path column keys like `unit.unit_name`, explicit `unique_id`, DRF's `{count, results}` shape), a best-effort Django token fetch added to `Login.jsx` alongside the existing PHP login, and a `/api` proxy in `vite.config.js`. Backend: `core/mongo_viewset.py`'s pagination now accepts `?page_size=`. Unit was the simple no-FK case, Item the harder nested-reference case (unit dropdown + `unit.unit_name` display column) — both share the new `mode="django"` path, so remaining modules are now mostly plumbing, not design. **Live verification (2026-07-03, after Atlas blocker resolved)**: full curl cycle against the real cluster — login → create Unit → list Units (paginated) → create Item referencing that Unit → retrieve → delete Item → delete Unit — all correct, including the nested `unit: {unique_id, unit_name}` shape on Item. Full backend suite still 69/69. **Still not verified**: pixel-level browser click-through (Chrome extension not connected in this environment) — the API contract itself is confirmed correct, so this is a lower-risk gap than the live-data one was. |
