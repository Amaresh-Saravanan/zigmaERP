...
switch ($action) {
            ...
            case 'datatable':
              ...
              if ($result->status) {
                  ...
                  foreach ($res_array as $key => $value) {
                      ...
                      $btn_update = '';
                      $btn_delete = '';
                      $status_update_delete_hide=status_update_delete_hide($value['batch_id1']);
                      if( $_SESSION['sess_user_id'] !="66604f07ae42a24843"){
                      if (($entry_date != $current_date) || ($entry_date == $current_date && $value['hatching_status'] == "<span>Completed</span>") || (!empty($status_update_delete_hide))) {
                        $btn_update = '';
                        $btn_delete = '';
                    } else {
  ...
  case 'delete':
    ...
    $action_obj = $pdo->update($table1, $columns1, $update_where1); //egg_process
    // print_r($action_obj);
    $action_obj = $pdo->update($table, $columns, $update_where);
    if ($action_obj->status) {
      $status = $action_obj->status;
...
function btn_delete_status1($folder_name = "", $unique_id = "", $batch_id1 = "")
{
  ...
  return $final_str;
    ...
    function status_update_delete_hide ($unique_id = "") {
    global $pdo;

    $table_name    = "frp_process";
    $where         = [];
    $table_columns = [
        "unique_id",
        "egg_batch_id"
